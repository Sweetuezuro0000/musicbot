...
from Zaid.MusicUtilities.tgcallsrun import (music, convert, download, clear, get, is_empty, put, task_done, ASS_ACC)
from Zaid.MusicUtilities.database.queue import (get_active_chats, is_active_chat, add_active_chat, remove_active_chat, music_on, is_music_playing, music_off)
...
from Zaid.config import LOG_GROUP_ID, DURATION_LIMIT, RESULT_PIC, PLAYLIST_PIC
...
from Zaid.MusicUtilities.helpers.inline import (play_keyboard, search_markup2, search_markup)
...
async def play(_, message: Message):
    ...
    if await is_on_off(1):
        if int(chat_id) != int(LOG_GROUP_ID):
...
async def popat(_,CallbackQuery): 
    ...
    if i == 1:
        ...
        await CallbackQuery.edit_message_text(
            f"6️⃣ <b>{title6[:25]}</b>\n  ┗  ⏳ <b>__Duration:__</b> {duration6}\n\n7️⃣ <b>{title7[:25]}</b>\n  ┗  ⏳ <b>__Duration:__</b> {duration7}\n\n8️⃣ <b>{title8[:25]}</b>\n  ┗  ⏳ <b>__Duration:__</b> {duration8}\n\n9️⃣ <b>{title9[:25]}</b>\n  ┗  ⏳ <b>__Duration:__</b> {duration9}\n\n🔟 <b>{title10[:25]}</b>\n  ┗  ⏳ <b>__Duration:__</b> {duration10}",    
            reply_markup=InlineKeyboardMarkup(buttons),
        )  
